...
"></i> Sorting Data</a>
              ...
              <table class="table table-striped table-bordered">
				...
				$sql = mysql_query("SELECT * FROM tbl_penanggung_jawab LEFT JOIN tbl_komunitas ON tbl_penanggung_jawab.id_komunitas = tbl_komunitas.id_komunitas LEFT JOIN tbl_anggota ON tbl_anggota.no_ktp = tbl_penanggung_jawab.no_ktp ORDER BY $orderBy $statusbaru LIMIT $posisi,5");
                while($hasil=mysql_fetch_array($sql)){
                ?>
                <tbody>
                  <tr>
...
"></i> Edit</button></a>
			  ...
			  <ul class="pagination">
				<?php
				$res    = mysql_query("SELECT * FROM tbl_penanggung_jawab LEFT JOIN tbl_komunitas ON tbl_penanggung_jawab.id_komunitas = tbl_komunitas.id_komunitas LEFT JOIN tbl_anggota ON tbl_anggota.no_ktp = tbl_penanggung_jawab.no_ktp ORDER BY $orderBy $statusbaru");
				$hitung = mysql_num_rows($res);
				$jum    = $hitung/5;
				$jumlah = ceil($jum);
				for($i=1; $i<=$jumlah; $i++){
					echo "<li><a href='v_pj.php?halaman=$i'>".$i."</a></li>";
				}
				?>
			  </ul>
              <?php
