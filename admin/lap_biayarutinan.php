...
<form style="margin:0px;" method="POST" action="cari_lapbiayarutinan.php">
			...
			<style type="text/css">
			  ...
			  <a target="_blank" href="report_biayarutinan.php"><button style="float:left;" class="btn btn-primary" style="margin:6px 15px 6px 0px;">Print Data</button></a>
			  <!-- ASCENDING -->
...
"></i> Sorting Data</a>
              ...
              <table class="table table-striped table-bordered">
				...
				$sql = mysql_query("SELECT * FROM tbl_detail_anggota LEFT JOIN tbl_komunitas ON tbl_detail_anggota.id_komunitas = tbl_komunitas.id_komunitas LEFT JOIN tbl_anggota ON tbl_anggota.no_ktp = tbl_detail_anggota.no_ktp ORDER BY $orderBy $statusbaru LIMIT $posisi,5");
				while($hasil = mysql_fetch_array($sql)){
                ?>
                <tbody>
                  <tr>
...
'></i> Hapus</button></a>";
			  ...
			  <ul class="pagination">
				<?php
				$res    = mysql_query("SELECT * FROM tbl_detail_anggota LEFT JOIN tbl_komunitas ON tbl_detail_anggota.id_komunitas = tbl_komunitas.id_komunitas LEFT JOIN tbl_anggota ON tbl_anggota.no_ktp = tbl_detail_anggota.no_ktp ORDER BY $orderBy $statusbaru");
				$hitung = mysql_num_rows($res);
				$jum    = $hitung/5;
				$jumlah = ceil($jum);
				for($i=1; $i<=$jumlah; $i++){
					echo "<li><a href='lap_biayarutinan.php?halaman=$i'>".$i."</a></li>";
				}
				?>
			  </ul>
			  <?php
